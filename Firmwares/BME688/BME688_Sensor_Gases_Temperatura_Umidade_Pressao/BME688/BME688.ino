...
void setup() {
  ...
  if (!bme.begin()) {
    Serial.println(F("Não foi possível encontrar o sensor BME680 / BME688, verifique as conexões!"));
    while (1);
  }
...
void loop() {
  ...
  if (endTime == 0) {
    Serial.println(F("Falha ao iniciar leiura :("));
    return;
  }
  Serial.print(F("Leitura começou em: "));
  Serial.print(millis());
  Serial.print(F(" e irá terminar em "));
  Serial.println(endTime);

  Serial.println(F("Você pode fazer outra coisas durante a medição do BME680 / BME688."));
  ...
  if (!bme.endReading()) {
    Serial.println(F("Falha para completar a leitura :("));
    return;
  }
  Serial.print(F("Leitura completa em "));
  ...
  Serial.print(F("Altitudade Aproximada = "));
